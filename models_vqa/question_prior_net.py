...
# The network that takes in the hidden state (containing .c and .h) of each layer of encoder and concat them
def question_prior_net(encoder_states, num_choices, qpn_dropout, hidden_dim=500, scope='question_prior_net', reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        # concate the LSTM states from all layers
        assert(isinstance(encoder_states, tuple))

        #encoder_states: contains .c. and .h variables
        #att_seq2seq.encoder_states[1].h:
        #< tf.Tensor 'neural_module_network/layout_generation/encoder_decoder/encoder/lstm/while/Exit_4:0'

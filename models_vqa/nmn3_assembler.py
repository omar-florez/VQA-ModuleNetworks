...
class Assembler:
    def __init__(self, module_vocab_file):
        ...
        #P = (num_vocab_nmn=13, state_size=3)
        #W = (state_size, num_vocab_nmn, num_constraints=4)
        #b = (num_vocab_nmn, num_constraints)
        ## the validity token list is
        #       XW - b >= 0
        #state x of [#att, #ans, T_remain]
    ...
    def _assemble_layout_tokens(self, layout_tokens, batch_idx):
        ...
        for t in range(len(layout_tokens)):
            ...
            if module_idx == self.EOS_idx:      #self.EOS_idx: 14
                break
            module_name = self.module_names[module_idx]                 #name of module
            expr = {'module': module_name,
                    'output_type': _module_output_type[module_name],    #type of output for this module: ans or att
                    'time_idx': t,
                    'batch_idx': batch_idx}

            input_num = _module_input_num[module_name]                  #number of inputs for this module
            ...
            for n_input in range(input_num-1, -1, -1):
                ...
                expr['input_%d' % n_input] = stack_top              #<--- tree generation
    ...
    def assemble(self, layout_tokens_batch):
        ...
        expr_list = [self._assemble_layout_tokens(layout_tokens_batch[:, n], n) for n in range(N)]

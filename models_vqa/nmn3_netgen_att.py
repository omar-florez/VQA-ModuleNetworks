    ...
    if isinstance(lstm_dim, list):  # Different layers have different dimensions
        ...
        for l in range(num_layers):
            ...
            if apply_dropout and l < num_layers-1:
                dropout_cell = tf.contrib.rnn.DropoutWrapper(lstm_cell, output_keep_prob=0.5)
            else:
...
class AttentionSeq2Seq:
    ...
    def _build_encoder(self, input_seq_batch, seq_length_batch, scope='encoder',
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            self.T_encoder = T
            self.N = N
            embedding_mat = tf.get_variable('embedding_mat', [self.encoder_num_vocab, self.encoder_embed_dim])
            ...
            encoder_outputs, encoder_states = tf.nn.dynamic_rnn(cell, embedded_seq, seq_length_batch, dtype=tf.float32,
            ...
            encoder_h_transformed = fc('encoder_h_transform', tf.reshape(encoder_outputs, [-1, lstm_dim]),
                                       output_dim=lstm_dim)
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            def loop_fn(time, cell_output, cell_state, loop_state):
                if cell_output is None:  # time == 0
                    #lstm_dim=1000
                    #(LSTMStateTuple(c=<tf.Tensor shape=(?, 1000) dtype=float32>, h=<tf.Tensor shape=(?, 1000) dtype=float32>),
                    # LSTMStateTuple(c=<tf.Tensor shape=(?, 1000) dtype=float32>, h=<tf.Tensor shape=(?, 1000) dtype=float32>))
                    next_cell_state = encoder_states                    #2 tuples of (c,h)
                    ...
                    # att_raw: [T, N, 1]
                    # self.encoder_h_transformed: [T, N, lstm_dim]
                    # W_a: [lstm_dim, lstm_dim]
                    # b_a: [lstm_dim]
                    # cell_output: [?, lstm_dim]
                    # v: [lstm_dim]

                    #Use broadcasting:
                    #[T, N, lstm_dim] = [?, lstm_dim] + [T, N, lstm_dim]
                    att_raw = tf.reduce_sum(tf.tanh(tf.nn.xw_plus_b(cell_output, W_a, b_a) +
                    ...
                    #collapse time in encoder into a single node (thought verctor?)
                    ...
                    if sampling:
                        #token_scores: (?, 15)      15 types of module names
                        #validity_mult: (?, 15)
                        ...
                        sampled_token = tf.cast(tf.reshape(tf.multinomial(token_scores_valid, 1), [-1]), tf.int32)
                        ...
                        is_sampled_valid = tf.reduce_any(tf.logical_and(sampled_mask, token_validity), axis=1)

                        # Fall back to max score (no sampling)
                        min_score = tf.reduce_min(token_scores)
                        token_scores_valid = tf.where(token_validity, token_scores, tf.ones_like(token_scores)*(min_score-1))
                    ...
                    neg_entropy = tf.reduce_sum(all_token_probs * tf.log(all_token_probs + (1-validity_mult)), axis=1)

                    # update states
                    updated_decoding_state = _update_decoding_state(decoding_state, predicted_token, self.P)

                    # the prediction is from the cell output of the last step
                    # timestep (t-1), feed it as input into timestep t

                    #next_input: (?, 300)   embed_dim_txt: 300
            ...
            # The RNN
            cell = _get_lstm_cell(num_layers, lstm_dim, apply_dropout)
            #returns tuple: emit_ta, final_state, final_loop_state
